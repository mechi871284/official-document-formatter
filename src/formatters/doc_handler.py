...
class DocFormatHandler(FormatHandler):
    ...
    通过win32com将旧版.doc格式转换为.docx，然后使用DOCX处理器进行格式化。
    需要安装pywin32库和Microsoft Word。
    """
    ...
    def validate_format(self, file_path: str) -> Tuple[bool, str]:
        ...
        if not self._win32_available:
            return False, "需要安装pywin32库和Microsoft Word才能处理.doc文件"
        try:
            import win32com.client
